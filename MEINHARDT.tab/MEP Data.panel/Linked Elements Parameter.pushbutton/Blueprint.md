# Linked Elements Parameter Transfer - Blueprint

## 1. Tool Identity
- Tool name: Linked Elements Parameter Transfer
- Platform: pyRevit (Python + Revit API + WPF/XAML)
- Ribbon location: MEINHARDT.tab > MEP Data.panel > Linked Elements Parameter
- Goal: transfer selected linked-source-element parameter values into selected host model elements for practical MEP coordination workflows.
...
4. User selects a linked source element type and picks/detects linked source element(s) from selected link in view.
5. Tool extracts source parameters (empty values hidden by default).
...
Window title: Linked Elements Parameter Transfer

### 5.1 Top Section
- Linked model dropdown.
- Detect Linked Elements / Pick Linked Element(s) button.
- Selected Source Info panel:
  - Source Category
  - Element Name/Number (when available)
  - Level
  - Link Name
