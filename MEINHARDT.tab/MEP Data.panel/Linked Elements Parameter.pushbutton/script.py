...
__title__ = "Linked Elements\nParameter Transfer"
__doc__ = "Select a linked source element and transfer source parameter values into selected host elements."
...
class LinkedRoomTransferWindow(WPFWindow):
    ...
    def __init__(self, xaml_file_name):
        ...
        self._build_source_element_type_list()
    ...
    def _build_source_element_type_list(self):
        """Populate source element type dropdown from categories present in loaded links."""
        self.source_category_items = []

        combo = getattr(self, "cmbSourceElementType", None)
        if combo is None:
            return
            ...
            for el in elements:
                try:
                    ...
                    if cat_id in categories_by_id:
                        continue
                    categories_by_id[cat_id] = cat.Name or "Category {0}".format(cat_id)
                except Exception:
                    continue

        self.source_category_items = [
            {"id": cid, "name": cname}
            for cid, cname in categories_by_id.items()
        ]
        self.source_category_items.sort(key=lambda x: (x["name"] or "").lower())

        room_id = int(BuiltInCategory.OST_Rooms)
        room_index = -1
        for i, item in enumerate(self.source_category_items):
            if item["id"] == room_id:
                room_index = i
                break

        combo.Items.Clear()
        for item in self.source_category_items:
            combo.Items.Add(item["name"])

        if self.source_category_items:
            combo.SelectedIndex = room_index if room_index >= 0 else 0

    def _selected_source_category_id(self):
        combo = getattr(self, "cmbSourceElementType", None)
        if combo is None:
            return int(BuiltInCategory.OST_Rooms)

        idx = combo.SelectedIndex
        if idx is None or idx < 0 or idx >= len(self.source_category_items):
            return int(BuiltInCategory.OST_Rooms)
        return int(self.source_category_items[idx]["id"])

    def _selected_source_category_name(self):
        combo = getattr(self, "cmbSourceElementType", None)
        if combo is None:
            return "Rooms"
        ...
        if idx is None or idx < 0 or idx >= len(self.source_category_items):
            return "Rooms"
        return self.source_category_items[idx]["name"]

    def _is_room_source_selected(self):
        return self._selected_source_category_id() == int(BuiltInCategory.OST_Rooms)
    ...
    def _reset_selected_room(self):
        ...
        self.selected_rooms = []
        self.selected_room_params = {}
        self.txtRoomInfo.Text = "No source element selected. Use Step 1 Detect or Pick Linked Elements."
    ...
    def _set_selected_rooms(self, room_items):
        """Persist selected linked source elements and use first item as mapping source."""
        ...
        if room_count > 1:
            header = "{0}\nSource elements selected: {1}".format(header, room_count)
    ...
    def _room_header_text(self, room, room_doc, link_inst):
        ...
        try:
            cat_name = room.Category.Name if room.Category is not None else "<No Category>"
        except Exception:
        ...
        if self._is_room_source_selected():
            return (
                "Source Category: {0}\n"
                "Room Name: {1}\n"
                "Room Number: {2}\n"
                "Level: {3}\n"
                "Link Name: {4}"
            ).format(cat_name, room_name, room_number, room_level, link_inst.Name)
        ...
        return (
            ...
            "Level: {3}\n"
            "Link Name: {4}"
        ).format(cat_name, room_name, room.Id.IntegerValue, room_level, link_inst.Name)
    ...
    def auto_detect_elements_click(self, sender, e):
        try:
            if not self._is_room_source_selected():
                forms.alert(
                    "Auto-detect by containment is available only when Source Element Type is Rooms.\n"
                    "For other source types, pick a linked source element in Step 1 and then transfer."
                )
                return
            ...
            if not self.room_detection_index:
                forms.alert(
                    ...
                    "Tip: try 'Entire Project' scope in Step 2B, or use Step 1 Detect Linked Elements to confirm links contain rooms."
    ...
    def _get_room_detection_mode(self):
        ...
        try:
            ...
            pass
        return "Auto-Detect Linked Elements In Current View"
    ...
    def _linked_element_matches_active_level(self, element, link_doc, tol=0.01):
        active_level = self._get_active_level()
        if active_level is None:
            return True
        ...
        try:
            level_id = getattr(element, "LevelId", None)
            if level_id is not None and getattr(level_id, "IntegerValue", -1) > 0:
                linked_level = link_doc.GetElement(level_id)
        except Exception:
            linked_level = None
        ...
        if linked_level is None:
            try:
                ...
                if p is not None and p.HasValue:
                    lvl_id = p.AsElementId()
                    if lvl_id is not None and lvl_id.IntegerValue > 0:
                        linked_level = link_doc.GetElement(lvl_id)
            except Exception:
                linked_level = None

        if linked_level is None:
            # Keep elements when level cannot be resolved, to avoid false negatives.
            return True

        try:
            if abs(float(linked_level.Elevation) - float(active_level.Elevation)) <= tol:
                return True
        except Exception:
            pass

        try:
            return (linked_level.Name or "").strip().lower() == (active_level.Name or "").strip().lower()
        except Exception:
            return False

    def _collect_linked_source_elements(self, links_to_scan, level_id=None):
        source_items = []
        selected_cat_id = self._selected_source_category_id()
        is_room_source = (selected_cat_id == int(BuiltInCategory.OST_Rooms))
        ...
        for link_inst in links_to_scan:
            ...
            if is_room_source:
                ...
                try:
                    source_elements = FilteredElementCollector(link_doc).WhereElementIsNotElementType()
                except Exception:
                    continue
            ...
            for source_element in source_elements:
                if source_element is None:
                    continue
                if not is_room_source:
                    try:
                        cat_id = source_element.Category.Id.IntegerValue if source_element.Category else None
                    except Exception:
                        cat_id = None
                ...
                if level_id is not None:
                    if is_room_source:
                        if not self._room_matches_active_level(source_element, link_doc):
                            continue
                    elif not self._linked_element_matches_active_level(source_element, link_doc):
                        continue
                source_items.append((link_inst, link_doc, source_element))
    ...
    def _auto_detect_rooms_in_link(self):
        """Auto-detect linked source elements by mode with optional selected-link filter."""
        detection_mode = self._get_room_detection_mode()
        selected_link = self._get_selected_link()
        source_label = self._selected_source_category_name()
        ...
        rooms = self._collect_linked_source_elements(links_to_scan, level_id=level_id)

        # Fallback: if no rooms found at level, retry without level filter.
        fallback_used = False
        if not rooms and level_id is not None:
            rooms = self._collect_linked_source_elements(links_to_scan, level_id=None)
            fallback_used = bool(rooms)

        if not rooms:
            forms.alert(
                "No linked source elements of type '{0}' were detected for the selected mode.\n"
                "Try 'Auto-Detect Entire Project' or keep link as 'All Linked Models'.".format(source_label)
            )
            return

        chosen = self._pick_representative_room(rooms)
        if chosen is None:
            forms.alert("No valid source element candidate found.")
            return

        # Keep all detected source elements and promote representative item for parameter list display.
        ordered = [chosen]
        for item in rooms:
        ...
        msg = (
            "Auto-detected {0} source element(s) of type '{1}' from {2} link(s).\n"
            "Selected source from: {3}"
        ).format(len(rooms), source_label, len(links_to_scan), link_inst.Name)
        if fallback_used:
            msg += "\nNote: no source elements were found on current level, so full-link scan was used."
        forms.alert(msg)
    ...
    def pick_linked_rooms_click(self, sender, e):
        selected_link = self._get_selected_link()
        selected_source_cat_id = self._selected_source_category_id()
        selected_source_cat_name = self._selected_source_category_name()

        self.Hide()
        try:
            picked_refs = uidoc.Selection.PickObjects(
                ObjectType.LinkedElement,
                "Pick one or more linked source elements in current view (ESC to finish)",
            )
        except Exception:
            self.Show()
            return
        ...
        for picked in picked_refs:
            ...
            if cat_id != selected_source_cat_id:
                skipped_not_source_type += 1
                continue
        ...
        if not room_items:
            forms.alert(
                "No linked source elements were picked. Please pick '{0}' elements only.".format(
                    selected_source_cat_name
                )
            )
            return
        ...
        msg = "Picked {0} linked source element(s).".format(len(room_items))
        if skipped_not_source_type:
            msg += "\nIgnored picks not '{0}': {1}".format(selected_source_cat_name, skipped_not_source_type)
    ...
    def add_update_mapping_click(self, sender, e):
        ...
        if not room_param:
            forms.alert("Pick a source parameter.")
            return
        if not target_param:
    ...
    def remove_mapping_click(self, sender, e):
        room_param = self.cmbMappingRoom.SelectedItem
        if not room_param:
            forms.alert("Pick a source parameter to remove mapping.")
            return
    ...
    def _write_transfer_log(self, lines):
        ...
        path = os.path.join(tempfile.gettempdir(), "LinkedElementsTransfer-{0}.log".format(ts))
    ...
    def transfer_click(self, sender, e):
        ...
        is_room_source = self._is_room_source_selected()

        if auto_room_mode and not is_room_source:
            auto_room_mode = False
            if getattr(self, "chkAutoRoomByElement", None) is not None:
                self.chkAutoRoomByElement.IsChecked = False
            forms.alert(
                "Per-element containment mode is available only for Rooms source type.\n"
                "Continuing with single selected linked source element mode."
            )
        ...
        if not auto_room_mode and self.selected_room is None:
            forms.alert("Select a linked source element first.")
            return
        if not self.selected_elements:
        ...
        tx = Transaction(doc, "Linked Elements Parameter Transfer")
        tx.Start()
        try:
        ...
        summary = [
            "Transfer Summary",
            "Source Type: {0}".format(self._selected_source_category_name()),
            "Mode: {0}".format("Auto detect room per element" if auto_room_mode else "Single selected source element"),
            "Source context: {0}".format(
        ...
        forms.alert("\n".join(summary), title="Linked Elements Parameter Transfer")
