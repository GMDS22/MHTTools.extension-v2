...
This tool will align viewports on selected sheets.
...
# ╩╩ ╩╩  ╚═╝╩╚═ ╩ ╚═╝ IMPORTS
# ====================================================================================================
from Autodesk.Revit.DB import (XYZ, BuiltInCategory, BuiltInParameter, ElementId, TemporaryViewMode, View, ViewPlan, ViewSheet, Transaction, TransactionGroup,
...
def ef_Transaction(doc, title, debug=True, exitscript=False):
    ...
    try:
        ...
        if debug:
            print("*" * 20)
            print("Exception occurred - transaction is being rolled back.")
...
def get_titleblock_on_sheet(sheet, uidoc=uidoc):
    all_titleblocks = FilteredElementCollector(uidoc.Document).OfCategory(
        BuiltInCategory.OST_TitleBlocks
    ).WhereElementIsNotElementType().ToElements()

    titleblocks_on_sheet = [tb for tb in all_titleblocks if tb.OwnerViewId == sheet.Id]
    if not titleblocks_on_sheet:
        print("***No TitleBlocks were found on given ViewSheet ({})***".format(sheet.SheetNumber))
        return None
    if len(titleblocks_on_sheet) > 1:
        print("***There is more than 1 TitleBlock on given ViewSheet ({})***".format(sheet.SheetNumber))
        return None
    return titleblocks_on_sheet[0]


def get_selected_sheets(given_uidoc=uidoc, exit_if_none=False, title=__title__,
                        label='Select Sheets', btn_name='Select Sheets'):
    selected_ids = list(given_uidoc.Selection.GetElementIds())
    selected_sheets = [
        given_uidoc.Document.GetElement(eid)
        for eid in selected_ids
        if type(given_uidoc.Document.GetElement(eid)) == ViewSheet
    ]

    if not selected_sheets:
        all_sheets = FilteredElementCollector(given_uidoc.Document).OfCategory(
            BuiltInCategory.OST_Sheets
        ).WhereElementIsNotElementType().ToElements()
        sheet_map = {
            "{} - {}".format(sheet.SheetNumber, sheet.Name): sheet
        ...
        if selected_names:
            selected_sheets = [sheet_map[name] for name in selected_names]
