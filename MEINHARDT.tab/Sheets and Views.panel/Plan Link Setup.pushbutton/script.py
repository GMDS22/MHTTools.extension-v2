...
def _clone_view_row(source_row, is_checked):
    clone = ViewRow.__new__(ViewRow)
    clone.Sheet = source_row.Sheet
    clone.View = source_row.View
    clone.IsChecked = is_checked
    clone.SheetNumber = source_row.SheetNumber
    clone.SheetName = source_row.SheetName
    clone.ViewName = source_row.ViewName
    clone.ViewType = source_row.ViewType
    clone.LevelName = source_row.LevelName
    clone.TemplateName = source_row.TemplateName
    clone.Display = source_row.Display
    clone.SearchText = source_row.SearchText
    return clone
...
class PlanLinkSetupWindow(forms.WPFWindow):
    def __init__(self, xaml_name):
        ...
        self._linked_view_choice_cache = {}
        self._last_clicked_sheet_node = None
        self._sheet_view_cache = {}
        self._view_link_cache = {}
        self._selected_sheets_cache = []
        self._selected_sheets_dirty = True
        self._last_link_signature = None
    ...
    def _init_static_controls(self):
        ...
        self.UI_linked_view.ItemsSource = [DisplayOption("<Select a source link first>")]
    ...
    def _invalidate_sheet_selection(self):
        self._selected_sheets_dirty = True
        self._last_link_signature = None

    def _get_selected_sheets(self):
        if not self._selected_sheets_dirty:
            return self._selected_sheets_cache
        ...
        self._selected_sheets_cache = selected
        self._selected_sheets_dirty = False
        return self._selected_sheets_cache

    def _sync_group_checkstates(self, nodes):
        def _walk(node):
            if node.IsSheet:
                return (1 if node.IsChecked else 0, 1)

            checked_count = 0
            total_count = 0
            for child in node.Children:
                child_checked, child_total = _walk(child)
    ...
    def _sync_sheet_group_states(self):
        self._sync_group_checkstates(self._full_tree_roots)
        if self._tree_roots is not self._full_tree_roots:
    ...
    def _load_sheets(self, initial_sheet_ids=None):
        ...
        self._invalidate_sheet_selection()
        self._sync_group_checkstates(self._full_tree_roots)
    ...
    def _apply_sheet_filter(self, raw_text):
        ...
        self._last_clicked_sheet_node = None
        if self._tree_roots is self._full_tree_roots:
            self._sync_sheet_group_states()
        else:
            self._sync_group_checkstates(self._tree_roots)
        self.UI_tree_sheets.ItemsSource = self._tree_roots
    ...
    def _get_cached_sheet_views(self, sheet):
        try:
            sheet_id = sheet.Id.IntegerValue
        except Exception:
            return []
        cached_rows = self._sheet_view_cache.get(sheet_id)
        if cached_rows is None:
            cached_rows = _collect_selected_sheet_views([sheet])
            self._sheet_view_cache[sheet_id] = cached_rows
        return cached_rows

    def _build_selected_view_rows(self):
        previous = {}
        ...
        seen_ids = set()
        for sheet in self._get_selected_sheets():
            for cached_row in self._get_cached_sheet_views(sheet):
                try:
                    view_id = cached_row.View.Id.IntegerValue
                except Exception:
                    continue
                if view_id in seen_ids:
                    continue
                seen_ids.add(view_id)
                fresh_rows.append(_clone_view_row(cached_row, previous.get(view_id, True)))

        fresh_rows.sort(key=lambda row: (row.SheetNumber, row.ViewName.lower()))
        return fresh_rows

    def _get_selected_view_signature(self):
        signature = []
        for row in self._all_view_rows:
            if not row.IsChecked:
                continue
            try:
                signature.append(row.View.Id.IntegerValue)
            except Exception:
                continue
        signature.sort()
        return tuple(signature)

    def _get_cached_link_rows_for_view(self, view):
        try:
            view_id = view.Id.IntegerValue
        except Exception:
            return []
        cached_rows = self._view_link_cache.get(view_id)
        if cached_rows is None:
            cached_rows = _collect_visible_link_options(view)
            self._view_link_cache[view_id] = cached_rows
        return cached_rows

    def _refresh_views(self):
        self._all_view_rows = self._build_selected_view_rows()
    ...
    def _refresh_links(self):
        signature = self._get_selected_view_signature()
        previous_id = None
        try:
        ...
        if signature != self._last_link_signature:
            collected = {}
            source_views = [row.View for row in self._get_selected_view_rows() if getattr(row, "View", None) is not None]
            if not source_views and revit.active_view is not None:
                source_views = [revit.active_view]

            for view in source_views:
                for row in self._get_cached_link_rows_for_view(view):
                    try:
                        key = row.LinkType.Id.IntegerValue
                    except Exception:
                        continue
            ...
            self._all_link_rows = list(collected.values())
            self._all_link_rows.sort(key=lambda item: (0 if item.IsArchitectural else 1, item.Name.lower()))
            self._last_link_signature = signature
    ...
    def _refresh_linked_view_choices(self):
        ...
        if source_link is None:
            self._linked_view_choices = [DisplayOption("<Select a source link first>")]
        ...
        if not link_doc:
            self._linked_view_choices = [DisplayOption("<Selected link is not loaded>")]
            self.UI_linked_view.ItemsSource = self._linked_view_choices
            self.UI_linked_view.SelectedIndex = 0
            self._update_counts()
            return
        ...
        try:
            link_type_id = source_link.LinkType.Id.IntegerValue
        except Exception:
            link_type_id = None

        choices = self._linked_view_choice_cache.get(link_type_id)
        if choices is None:
            ...
            if link_type_id is not None:
                self._linked_view_choice_cache[link_type_id] = choices
        if not choices:
            self._linked_view_choices = [DisplayOption("<No printable linked views found>")]
            self.UI_linked_view.ItemsSource = self._linked_view_choices
            self.UI_linked_view.SelectedIndex = 0
            self._update_counts()
            return
    ...
    def _invalidate_link_cache_for_views(self, views):
        for view in views:
            try:
                view_id = view.Id.IntegerValue
            except Exception:
                continue
            if view_id in self._view_link_cache:
                del self._view_link_cache[view_id]
        self._last_link_signature = None

    def _get_direct_selected_views(self):
        views = []
        seen_ids = set()
        for row in self._get_selected_view_rows():
            view = getattr(row, "View", None)
            if view is None:
                continue
            try:
    ...
    def _apply_fixed_custom_settings(self, settings, use_view_filters_by_host):
        ...
        success, message = _apply_link_visibility_property(settings, "ObjectStyles", "Custom / None")
        if success:
            applied.append("Model and annotation categories set to Custom")
        elif message:
    ...
    def _apply_to_targets(self):
        ...
        ordered = [
            ...
            "Model and annotation categories set to Custom",
        ...
        if clipboard_ok:
            self.UI_feedback.Text = "Applied to {} target(s). Action list copied to clipboard.".format(len(targets))
        elif clipboard_error:
            self.UI_feedback.Text = "Applied to {} target(s). Could not copy to clipboard: {}".format(len(targets), clipboard_error)
        self._invalidate_link_cache_for_views(self._get_direct_selected_views())
        self._refresh_links()
    ...
    def button_remove_selected_link(self, sender, event_args):
        source_link = self.UI_link_list.SelectedItem
        if source_link is None:
            forms.alert("Select the link to remove first.", title=__title__)
            return

        target_views = self._get_direct_selected_views()
        if not target_views:
            forms.alert("Check at least one placed view first.", title=__title__)
            return
        ...
        confirmation = (
            "Selected views: {}\n"
            "Selected link: {}\n\n"
            "Proceed to hide this link from the checked views?"
        ).format(len(target_views), source_link.Name)
        if not forms.alert(confirmation, title=__title__, yes=True, no=True):
            return
        ...
        already_hidden = 0
        skipped = []
        hidden_ids = List[DB.ElementId]([source_link.LinkType.Id])

        with revit.Transaction("Remove Selected Link From Views"):
            for view in target_views:
                view_name = getattr(view, "Name", "<Unnamed View>")
                try:
                    if not source_link.LinkType.CanBeHidden(view):
                        skipped.append("{} | Selected link cannot be hidden in this view".format(view_name))
                        continue
                except Exception as exc:
                    skipped.append("{} | Could not test hide support: {}".format(view_name, exc))
                    continue

                try:
                    if source_link.LinkType.IsHidden(view):
                        already_hidden += 1
                        continue
                except Exception:
                    pass
                ...
                try:
                    view.HideElements(hidden_ids)
                    removed_count += 1
                except Exception as exc:
                    skipped.append("{} | Hide failed: {}".format(view_name, exc))

        self._invalidate_link_cache_for_views(target_views)
        self._refresh_links()

        summary_lines = [
            "Remove Selected Link",
            "Checked views: {}".format(len(target_views)),
            "Selected link: {}".format(source_link.Name),
            "",
            "Results:",
            "- Hidden in {} view(s)".format(removed_count),
            "- Already hidden in {} view(s)".format(already_hidden),
        ]
        if skipped:
            summary_lines.append("")
            summary_lines.append("Not applied:")
            summary_lines.extend("- {}".format(line) for line in skipped)

        summary_text = "\n".join(summary_lines)
        self.UI_feedback.Text = "Removed selected link from {} view(s).".format(removed_count)
        self._set_status("Removed selected link from {} view(s).".format(removed_count))
        self.UI_log.Text = summary_text
